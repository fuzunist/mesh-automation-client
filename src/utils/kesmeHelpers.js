...
export const printTable = (setIsPrinting, tableId) => {
  ...
  const changeBackgroundColor = (color, tableId) => {
    ...
    document.querySelectorAll(selector).forEach((el) => (el.style.backgroundColor = color));
  };
  ...
  setTimeout(() => {
    ...
    html2canvas(table).then((canvas) => {
      ...
      newWindow.document.title = tableId === "kesmeTable" ? "KESME BİLGİLERİ TABLOSU" : "SİPARİŞ BİLGİLERİ TABLOSU";
      newWindow.print();
      newWindow.close();
...
export function optimizeMeshProduction(orders) {
  ...
  let originalTotalLength = orders.reduce((total, order) => total + order[0] * order[1], 0);
      ...
      for (let length in orderDict) {
          ...
          if (length < target && quantity > 0) {
              ...
              for (let addLength in orderDict) {
                  ...
                  if (addLength <= target - combinedLength && addQuantity > 0 && extraLength >= minExtra && extraLength <= maxExtra) {
                      combinedLength += addLength;
                      orderDict[addLength] -= 1;
                      return true;
                  }
  ...
  let optimizedTotalLength = optimizedOrders.reduce((total, order) => total + order[0] * order[1], 0);
